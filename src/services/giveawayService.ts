...
export default class GiveawayService {
	...
	public async addAParticipant(guildId: string, messageId: string, userId: string): Promise<Giveaway> {
		...
		return await this.manipulateParticipants(guildId, messageId, addNewUser)
	}
	...
	public async removeAParticipant(guildId: string, messageId: string, userId: string) {
		const removeAUser = (giveaway: Giveaway): Giveaway => {
	 ...
	 * @private
	 */
	private async manipulateParticipants(guildId: string, messageId: string, callback: (giveaway: Giveaway) => Giveaway): Promise<Giveaway> {
		const currentGiveawayKey = `${this.giveawaysKey}:${guildId}:${messageId}`;
		let giveaway: Giveaway = (await this.dbClient.get(currentGiveawayKey) as Giveaway);
		if (!giveaway.stillRunning) {
			throw new EnduserMessageError('This giveaway is now over.');
		}
		const result = callback(giveaway);
		console.log(giveaway);
		await this.dbClient.set(currentGiveawayKey, result);
		return result;
	}
